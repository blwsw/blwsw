<template>
  <div class="app-container" style="padding: 0;margin: 0">
    <div>
      ...
      <el-table id="tableList4" ref="tableList4" :height="pageH" fit highlight-current-row :data="list">
        ...
        <el-table-column min-width="90px" align="center" label="泄露电流1 (mA)">
          <template slot-scope="scope">
            <span>{{ scope.row.LCurrent1 }}</span>
          </template>
        </el-table-column>

        <el-table-column min-width="90px" label="泄露电流2 (mA)" show-overflow-tooltip>
          <template slot-scope="{row}">
            <span>{{ row.LCurrent2 }}</span>
          </template>
        </el-table-column>
        <el-table-column min-width="90px" label="泄露电流3 (mA)" show-overflow-tooltip>
          <template slot-scope="{row}">
