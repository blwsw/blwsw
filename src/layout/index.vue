...
export default {
  ...
  methods: {
    ...
    setOnmessageMessage (event) {
      this.reset(),this.start(),
      // 根据服务器推送的消息做自己的业务处理
      console.log("服务端返回：" + event.data.length)
      if ("ping"!=event.data && event.data ){
        ...
        //  store.dispatch('app/setReals',newData)
          ...
          **/
          ...
          this.$router.go(0);
        }else if ('TJDATA' == redata.type) {
          store.dispatch('app/setNewData',redata)
        }
        else{
          var newData = redata.data;
          //console.log(newData)
    ...
    reconnect:function(e) {
      ...
      this.lockReconnect || (this.lockReconnect = !0, setTimeout((function() {
        t.websocket = new WebSocket(process.env.VUE_APP_WS_URS + t.$store.state.user.userId), t.initWebSocket(), t.lockReconnect = !1
      }), 2e3))
    },
    reset:function() {
    ...
    start:function() {
      ...
      this.timeoutObj = setTimeout((function() {
        e.websocket.send('ping'), console.log('ping! 后端websocket 验证是否中断'),
